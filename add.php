...
********************************/
    ...
    {   
        ...
        $today = getdate();
        if ($today == NULL)
            apologize("Error. Try again later.");
        ...
        // show add form
        render("templates/addform.php", array("dates" => $date, "title" => "Add Event"));
    }
    ...
    else
    {
		...
		if ($_POST["action"] == "submit")
		{
			...
			if ($_POST["date"] == "Date")
				apologize("You must select a date."); 
			else if (empty($_POST["title"]))
				apologize("You must give a title.");
			else if ($_POST["hour"] == "Hour")
				apologize("You must select a time."); 
			else if ($_POST["minute"] == "Min")
				apologize("You must select a time.");
			else if ($_POST["ampm"] == "AM/PM")
				apologize("You must select a time."); 
			else if (empty($_POST["building"]))
				apologize("You must select a building."); 
			...
			else
			{
				...
				&& !preg_match('/\b(\w*ass\w*)\b/', $_POST['title'], $matches))
				{
					...
					query("INSERT INTO emails (id) VALUES (NULL)");
					$ids = query("SELECT * FROM emails ORDER BY id DESC");
					$id1 = $ids[0]['id'];
					...
					query("UPDATE emails SET subject=?, html_path=?, parsed=1, freefood=1 WHERE id=?", 
					...
					{
						query("INSERT INTO food_info (id, title, month, day, year, hour, minute, ampm, building, 
					...
					success("Thank you for submitting your event.");
				}
				
				else
					apologize("Your event was rejected by our automatic moderator. If you believe this was
		...
		{
			$formatted = str_replace("\n", "<br/>", $_POST["comments"]);
			render("templates/preview.php", array("title" => $_POST["title"], "comments" => $formatted));
		}
    }
